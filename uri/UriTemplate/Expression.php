...
final class Expression implements IteratorAggregate
{
    ...
    {
        return match (true) {
            $this->operator->isQuery() => $this->extractNamedQueryValue($value),
            $this->operator->isNamed() => $this->extractNamedValue($value),
            default => $this->extractPositionalValue($value),
        };
    }
     ...
     */
    private function extractNamedQueryValue(string $value): ExtractionResult
    {
        /** @var non-empty-string $separator */
        $separator = $this->operator->separator();
        $components = '' === $value ? [] : explode($separator, $value);
        $matched = $this->matchNamedQueryValues($components);
     ...
     */
    private function matchNamedQueryValues(array $components): ?array
    {
