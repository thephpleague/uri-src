    ...
    private function matchNamedValues(
        ...
        if (null === $pattern->exploded) {
            if (
                ...
                $matched = $this->matchNamedValues(
                    $components,
                    $componentOffset + 1,
                    $varSpecifierOffset + 1,
                );
        ...
        if ($componentOffset < $componentCount) {
            ...
            for ($length = 1; $length <= $maximum; ++$length) {
                if (1 !== preg_match(
                    '/\A'.$componentPattern.'\z/',
                    $components[$componentOffset + $length - 1],
                )) {
                    break;
                }
                ...
                if (null !== $matched) {
                    return [
                        implode(
                            $this->operator->separator(),
                            array_slice($components, $componentOffset, $length),
                        ),
                        ...$matched,
                    ];
                }
