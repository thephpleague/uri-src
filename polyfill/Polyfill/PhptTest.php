...
class PhptTest extends TestCase
{
    protected function setUp(): void
    {
        if (\PHP_VERSION_ID >= 80600) {
            $this->markTestSkipped('The Builder polyfill is only used on PHP < 8.6.');
        }
    }

    #[DataProvider('providePhptFiles')]
    public function testPhpt(string $file): void
    {
    ...
    public static function providePhptFiles(): array
    {
        ...
        foreach (new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($baseDir)) as $file) {
            if ($file->isFile() && $file->getExtension() === 'phpt') {
                $files[substr($file, 1 + strlen($baseDir), -5)] = [$file->getPathname()];
            }
        }
