...
The `Builder` implements the `Conditionnable` interface to improve URI building.
...
And also provides the `tap()` method.
...
the URI being constructed can be safely build when calling the build method.

```php
new Builder()->scheme('https')->validate();
