...
~~~php
$uri = Uri::new('mailto://thephpleague.com/path/to?here#content');
...
These extra validation rules are triggered only when the URI is absolute and the scheme is detected.
Otherwise, only basic RFC3986 rules are taken into account.
</p>
...
~~~php
$uri = Uri::new('http://uri.thephpleague.com/');
echo $uri->withHost(null)->withScheme(null);
// will throw a League\Uri\Exceptions\SyntaxError
// you cannot remove the Host if a scheme is present
~~~
...
It cannot contain a query and or a fragment component.

<p class="message-notice">Adding contents to the fragment or query components throws an <code>UriException</code> exception</p>

~~~php
$uri = Uri::new('ftp://thephpleague.com/path/to/image.png;type=i');
$uri->withQuery('p=1'); // will throw an League\Uri\Exceptions\SyntaxError
~~~

<p class="message-info">Apart from the fragment requirements, FTP URIs share the same validation rules as HTTP(S) URIs.</p>

### WS(S) validation

It cannot contain a fragment component as per [RFC6455](https://tools.ietf.org/html/rfc6455#section-3).

<p class="message-notice">Adding contents to the fragment component throws an <code>UriException</code> exception</p>

~~~php
$uri = Uri::new('wss://thephpleague.com/path/to?here#content');
// will throw an League\Uri\Exceptions\SyntaxError
~~~

<p class="message-info">Apart from the fragment requirements, WSS URIs share the same validation rules as HTTP(S) URIs.</p>

### DATA validation

Even though all URI properties are defined and accessible attempt to set any component other than the
path will result in the object throwing a `SyntaxError` exception. As adding data to these URI parts
will generate an invalid Data URI.
...
<p class="message-info">Available since version <code>7.6</code></p>

URN validation is improved; and they are now all validated against [RFC8141](https://datatracker.ietf.org/doc/html/rfc8141). 

~~~php
$uri = Uri::new('urn://example.com'); // will throw a League\Uri\Exceptions\SyntaxError
~~~
...
The `Uri` class has two methods to compare URI:

- `Uri::isSameDocument` Tells whether the given URI object represents the same document according to RFC3986 rules
- `Uri::equals` allows fine-tuning the equivalence check against the full URI or excluding its fragment component.

To control its comparison algorithm, the `equals()` method uses the `League\Uri\UriComparisonMode` enum.
The Enum has two possible cases : `UriComparisonMode::IncludeFragment` and `UriComparisonMode::ExcludeFragment`.
By default, if not specified, the fragment component is not taken into account to align with
RFC3986 specifications and returns the same result as using `Uri::isSameDocument`.

~~~php
use League\Uri\Uri;
use League\Uri\UriComparisonMode;

$uriA = Uri::new("//example.com?foo=bar#🏳️‍🌈");
$uriB = Uri::new("//exAMpLE.com?foo=bar#🍣🍺);
$uriA->equals($uriB); //returns true
$uriA->equlas($uriB, UriComparisonMode::IncludeFragment); // returns false
$uriA->isSameDocument($uriB); // returns true
~~~

Since the equivalence is done by comparing normalized URI string, both methods allow comparing against
any URI object or valid string representations. If the string is not a valid URI the comparison will
silently fail and return `false`.

~~~php
$uriA = Uri::new("//example.com?foo=bar#🏳️‍🌈");
$uriA->equals("//exAMpLE.com?foo=bar#🍣🍺); //returns true
~~~
