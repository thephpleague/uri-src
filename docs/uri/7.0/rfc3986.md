...
The `Builder` also implements the `Conditionnable` interface to improve URI building.

```php
echo new Builder()
    ->path('path/to/whatever')
    ->when(
        str_contains('www.example.com', 'www'), 
    ...
    ->build()
    ->toString();
// returns "https://www.example.com/path/to/whatever"   
```
