...
<p class="message-info">If no input is provided an instance representing the empty string is returned.</p>
...
<p class="message-info">Apart from the fragment requirements, FTP URIs share the same [validation rules](/7.0/uri/schemes/http/#validation) as Http URIs.</p>
...
~~~php
$uri = Uri::fromPath('path/to/my/png/image.png');
