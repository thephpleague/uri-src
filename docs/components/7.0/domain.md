...
public static Domain::fromALabels(Stringable|string ...$labels): self
~~~
...
Domain::new('example.com')->value(); //returns 'example.com'
Domain::new(new Host('bébé.be'))->value(); //returns 'xn--bb-bjab.be'
Domain::fromUri("http://www.example.com/path/to/the/sky")->value(); //return 'www.example.com';
Domain::fromAuthority("user:pass@www.example.com:82")->value(); //return 'www.example.com';
~~~
...
$fqdn = Domain::fromLabels('', 'com', 'example', 'shop');
echo $fqdn; //display 'shop.example.com.'

Domain::fromLabels('0.1', '127.0');
...
$host = Domain::fromLabels('be', 'bébé');
...
~~~php
$host = Domain::new('secure.example.com');
$host->labels(); //return  ['com', 'example', 'secure'];

$fqdn = Domain::new('secure.example.com.');
...
~~~php
$host = Domain::new('uk.example.co.uk');
...
~~~php
$host = Domain::new('toto')->append('example.com');
...
~~~php
$host = Domain::new('example.com')->prepend('toto');
echo $host; //return toto.example.com
~~~
...
~~~php
$host    = Domain::new('foo.example.com');
